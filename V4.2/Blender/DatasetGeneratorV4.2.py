...
for frame in range(FRAME_START, FRAME_END+1):   # Iterate over each frame in the range
    ...
    for i in range(shapecount):           
        encdec.encode(encdec.float_to_uint24(ob.data.shape_keys.key_blocks[defs.shape_defs[defs.shape_index[i]]].value), i + 2) # Reset compositor data
        bpy.data.scenes["Scene"].node_tree.nodes["Group.002"].inputs[i + 2].keyframe_insert(data_path="default_value", frame=frame) # Write zeros to compositor data
    
    #for i in range(shapecount):             # Write zeros to compositor data
    #    bpy.data.scenes["Scene"].node_tree.nodes["Group.002"].inputs[i + 2].keyframe_insert(data_path="default_value", frame=frame)
    '''
    for i in range(len(values)):    # Iterate over each shape key and set frame
        key = ob.data.shape_keys.key_blocks[names[i]]   
        key.value = values[i]   # Sets the shapekey value 
    ...
    bpy.data.scenes["Scene"].node_tree.nodes["Group.002"].inputs[1].keyframe_insert(data_path="default_value", frame=frame) # Sets shape count
    '''
    
    for i in range(shapecount):     #
        encdec.encode(encdec.float_to_uint24(ob.data.shape_keys.key_blocks[defs.shape_defs[defs.shape_index[i]]].value), i + 2) # Sets compositor values
    for i in range(shapecount):
        bpy.data.scenes["Scene"].node_tree.nodes["Group.002"].inputs[i + 2].keyframe_insert(data_path="default_value", frame=frame) # Sets the now current values as a keyframe
