...
class ShapeSetter():
    ...
    def process_exclusives(self, defs):         
        ...
        if (defs.exclusives[2][12] in self.selected_shapes_index) and not defs.exclusives[2][1] in self.selected_shapes_index: 
            ...
            self.selected_shapes.append(0.0)
        '''
        if defs.exclusives[2][0] in self.selected_shapes_index or defs.exclusives[2][2] in self.selected_shapes_index or defs.exclusives[2][3] in self.selected_shapes_index or defs.exclusives[2][4] in self.selected_shapes_index or defs.exclusives[2][5] in self.selected_shapes_index or defs.exclusives[2][6] in self.selected_shapes_index or defs.exclusives[2][7] in self.selected_shapes_index or defs.exclusives[2][8] in self.selected_shapes_index or defs.exclusives[2][9] in self.selected_shapes_index or defs.exclusives[2][10] in self.selected_shapes_index or defs.exclusives[2][11] in self.selected_shapes_index or defs.exclusives[2][12] in self.selected_shapes_index in self.selected_shapes_index and (self.selected_shapes[self.selected_shapes_index.index(defs.exclusives[2][1])] <= 0.55):
    ...
    def generate_example(self, DefsClass, range, classExample):
        ...
        while len(self.possible_shapes) > 0:
            if 0.75 >= random.uniform(0,1):     # 75% chance for shape to be set
                self.selected_shapes.append(clamp(random.uniform(-0.1, self.selected_shapes[0] * 0.6), 0, 1))
